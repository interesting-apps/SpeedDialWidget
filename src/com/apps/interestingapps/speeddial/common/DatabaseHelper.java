	...
	public void createDataBase() throws IOException {
		...
		if (!dbExist) {
			try {
				...
				openDatabase();
				try {
					if (SpeedDialUtilities.isFreeAppInstalled(context)) {
						ContentResolver cr = context.getContentResolver();
						Cursor cursor = cr.query(CONTENT_URI, null, null, null, null);
						Log.d("DatabaseHelper", "cursor.getCount()="
								+ cursor.getCount());
						while (cursor != null && cursor.moveToNext()) {
							...
							int speedDialNumber = cursor
									.getInt(cursor
											.getColumnIndex(SpeedDialConstants.SPEED_DIAL_NUMBER_COLUMN));
							addRecord(name, phoneNumber, speedDialNumber);
						}
						Toast.makeText(context,
								"Successfully copied data from the free app",
								Toast.LENGTH_SHORT).show();
						cursor.close();
					}
				} catch (Exception e) {
					Log.i("DatabaseHelper", "Unable to copy data from the free app."
